...
class Main {
	...
	public static void main(String[] args) {
		...
		while (!linha.equals("0 0")) {
			...
			BigInt a = new BigInt(tks.nextToken());
			BigInt b = new BigInt(tks.nextToken());
			
			System.out.println(countFibo(a, b));
	...
	static int countFibo(BigInt inicio, BigInt fim) {
		BigInt a = new BigInt(1);
		BigInt b = new BigInt(2);
		
		int cont = 0;
		
		while (a.compareTo(fim) <= 0) {
			
			if (a.compareTo(inicio) >= 0) cont++;
